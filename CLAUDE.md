...
- `src/ui/utils/graphStyles.ts` — Full Cytoscape stylesheet (selectors for nodes, edges, hover, impact)
...
### Data Flow
`graph.json` → `buildElements()` → Cytoscape init → `layoutstop` → interactions/highlighting

### Edge Style (JetBrains-style)
- No convergence nodes or bundling algorithms — Cytoscape bezier handles visual separation
- Key properties: `curve-style: bezier`, `control-point-step-size: 60`, `opacity: 0.38`
- Parallel edges curve apart naturally, brain perceives bundles where edges cluster
- Hover: connected edges highlight to `opacity: 1, width: 3`
...
3. Edge animation starts (dashed line offset)
4. Viewport animates to affected nodes
